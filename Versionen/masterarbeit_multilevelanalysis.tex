...
In den letzten Abschnitten wurde das Konzept und die Anwendung von hierarchsichen linearen Modellen in R erläutert. Im zweiten Teil dieser Arbeit wird nun eine Simulationsstudie durchgeführt, um vorhandene Forschungsergebnisse zu replizieren und aufzuzeigen, dass es Situationen gibt, bei denen es zu massiven Fehlentscheidungen kommen kann, wenn die hierarchische Struktur in der Analyse vernachlässigt wird. Anschliessend an die Diskussion dieser Studie wird eine Shiny App \citep{shiny} vorgestellt, die im laufe dieser Arbeit programmiert wurde und mit der es Nutzern möglich sein wird, die Simulationsstudie aus dieser Arbeit selbst durchzuführen.

\subsection{Herleitung der Forschungsfrage}
Wie bereits in der Einleitung angedeutet, gibt es in der psychologischen Forschung viele hierarchische Datenstrukturen (cite). Der Einfluss dieser Datenstrukturen auf die Analyse wurden konzeptionell in den letzten Abschnitten diskutiert und vorgestellt. In diesem Abschnitt geht es nun, um die wissenschaftliche Replikation und Überprüfung dieses Einflusses. Vor allem möchten wir den Fokus auf die Unterschiede zwischen der Analyse mittels normaler linearer Regression und hierarchischer linearer Regression legen. Dabei wurde in einigen Studien gezeigt, dass der Unterschied der Regressionskoeffizienten bezüglich dieser beiden Analysemethoden relativ gering ist (cite). Das heisst, dass beide Methoden die grösse des Effekts einer Behandlung oder den Achsenabschnitt relativ genau schätzen können. Allerdings ist eine genaue Schätzung des Regressionskoeffizienten nicht ausreichend, um zu bestimmen, ob der Effekt auch signifikant ist. Um das zu überprüfen wird üblicherweise ein \textit{t} Test durchgeführt. Die Prüfgrösse des \textit{t} Tests wird über das Verhältnis zwischen des geschätzten Koeffizienten und des Standardfehlers bestimmt. Wird beispielsweise ein Standardfehler kleiner geschätzt als der wahre Standardfehler, steigt die Prüfgrösse an und die Rate in der die Nullhypothese abgelehnt wird, nimmt zu. Folglich führt eine Unterschätzung des Standardfehlers zu einer erhöhten Fehler Typ 1 Rate und eine Überschätzung zu sehr konservativen Testergebnissen. Daher ist eine genau Schätzung des Standardfehlers umso wichtiger, da dieser massgeblich zur Testung des Effekts beiträgt. Werden nun aus 100 Schulklassen 10 Schulkinder ausgewählt, würde eine normale lineare Regression mit einer Stichprobengrösse von 1000 arbeiten und eine hierarchische lineare Regression nur mit einer Stichprobengrösse von 100. Da der Standardfehler in einem direkten Zusammenhang mit der Stichprobengrösse steht, können diese beiden Methoden zu unterschiedlichen Standardfehlern und folglich unterschiedlichen Prüfgrössen für den \textit{t} Test gelangen. 

Neben der Prüfgrösse ist auch die Anazahl an Freiheitsgrade relevant, um die Signifikanz eines Effekts zu überprüfen. Während bei der normalen linearen Regression die Anzahl Freiheitsgrade durch $N + p - 1$ bestimmt wird, wobei $N$ die Stichprobengrösse und $p$ die Anzahl Parameter im Modell sind, ist die Berechnung der Freiheitsgrade in hierarchischen linearen Modellen einiges aufwändiger und auch heute noch ein aktuelles Forschungsthema. Dabei werden zur Zeit häufig die Satterthwaite (Satterthwaite, 1941) oder die Kenward-Roger Methode (Kenward Roger, 1997) verwendet, um die Freiheitsgrade eines hierarchischen linearen Modells zu berechnen. 

Aufgrund dieser Gegebenheiten ist es also wichtig, dass die verwendete Analysemethode der Struktur des Datensatzes gerecht wird, um falsche Schätzungen zu vermeiden. Mcneish konnte in seiner Studie zeigen, dass mit zunehmender Intraklassen Korrelation die Schätzung des Standardfehlers ungenauer wird. Diese Erkenntnis entspricht den Berechnungen die aus dem Artikel von (cite comparing) hervorgehen. In diesem Artiekl differenzieren die Autoren zwischen zwei Studiendesigns, die in der Praxis häufig eingesetzt werden. Zum einen kann eine Intervention auf Level-1 durchgeführt werden und zum anderen kann man die Randomisierung auf Level-2 durchführen. In beiden Designs konnten die Autoren zeigen, dass der geschätzte Standardfehler einer Intervention mittels normaler lineare Regression unter- oder überschätzt wurde. Da die Autoren in diesem Artikel keine Simulationsstudie durchgeführt haben, sollten ihre Ergebnisse noch in einer Simulationsstudie überprüft werden. Folglich wird in der aktuellen Simulationsstudie versucht die Ergebnisse von (parameter effizienz) bezüglich der Schätzung der Koeffizienten zu replizieren und die Aussagen von (comparision) bezüglich des Schätzers des Standardfehlers zu überprüfen.

\subsection{Design der Simulationsstudie}
In dieser Simulationsstudie wurden die selben beiden Designs eingesetzt, die auch (cite comparing) verwendet haben. Dabei wird im ersten Design die Intervention auf Level-1 durchgeführt. Das bedeutet, dass in jeder Schulklasse die Hälfte der Schulkinder zufällig einer Intervention zugewiesen wird und die andere Hälfte gehört zur Kontrollgruppe. Beim zweiten Design wird die Intervention zufällig auf alle Klassen verteilt. Folglich befindet sich die Intervention hier auf Level-2. 

Für die Durchführung der Simulationsstudie wurden für jedes Design Daten basierend auf den folgenden Gleichungen simuliert:
\begin{equation} 
\begin{split}
\text{Design 1:}\\	
 \text{Level 1:}  \qquad y_{ji} & = \beta_{0j} + \beta_{1}x_{ij} + \epsilon_{ij}\\
