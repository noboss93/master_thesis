...
\subsubsection{Berechnung der Intraklassen Korrelation in R}
...
Bis jetzt haben wir uns nur mit Level-1 Variablen beschäftigt. Möchten wir dem Modell die Anzahl Fenster im Klassenzimmer als Level-2 Variable hinzufügen, stellt sich die Frage, ob man eine \textit{Cross-Level} Interaktion zulassen möchte oder nicht. In der folgenden Gleichung wurde die Variable mit einer \textit{Cross-Level} Interaktion hinzugefügt, um nicht nur den Achsenabschnitt sondern auch die Steigung besser zu beschreiben:
\begin{equation} 
\begin{split}	
 \text{Level 1:}  \qquad y_{ji} & = \beta_{0j} + \beta_{1} \cdot \text{uebungen}_{ij} + \epsilon_{ij}\\
...
\end{split}	
\end{equation} 
In R ist das Hinzufügen von Level-2 Variablen relativ einfach, da eine Level-2 Variable genau gleich, wie eine Level-1 Variable hinzugefügt werden kann. Folgend wurden gleich zwei Modelle geschätzt, eines ohne und eines mit \textit{Cross-Level} Interaktion.
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{anova}\hlstd{(m_uncorr, m_lvl2, m_cross)}
\end{alltt}
\begin{verbatim}
## Data: beispiel_data
## Models:
