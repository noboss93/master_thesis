...
Dabei wird in \texttt{formula} die gewünschte Formel des Modells eingegeben, bei \texttt{data} wird der Datensatz festgelegt anhand das Modell geschätzt werden soll und bei \texttt{REML} wird durch einen logischen Operator (\textit{TRUE} oder \textit{FALSE}) eingestellt, ob das Modell mit \textit{Maximum Likelihood} (ML) oder mit \textit{Restricted Maximum Likelihood} (REML) geschätzt werden soll. In unserem Fall ist diese Einstellung vor allem beim Vergleich von Modellen wichtig. Da bei Modellen die mit REML geschätzt wurden nur die zufälligen Effekte miteinander verglichen werden können. Wenn man also feste als auch zufällige Effekte vergleichen möchte, sollte man Modelle mit ML schätzen \citep{PEUGH201085}. 
...
Bis zum Term innerhalb der Klammer ist die Syntax von \texttt{lmer()} genau die gleiche wie bei \texttt{lm()}. Innerhalb der Klammern können nun die zufälligen Effekte und die variierende Gruppe festgelegt werden. Dabei kann innerhalb der Klammern eingestellt werden, ob der Achsenabschnitt und die Steigung korrelieren sollten oder nicht. In Tabelle \ref{tab:lmersyntax} findet man einen Überblick über die möglichen Interaktionen, die man innerhalb der Klammer festlegen kann und die für uns relevant sind\footnote{Weitere Funktionen und eine ausführliche Beschreibung des Paketes \texttt{lme4} können in Bates et al. \citeyearpar{batesetal2015lme4} nachgeschlagen werden.}.
\begin{table}[ht] 
\centering
...
\subsubsection{Interpretation eines Outputs}
...
\hlstd{beispiel_model} \hlkwb{<-} \hlkwd{lmer}\hlstd{(punktzahl} \hlopt{~} \hlstd{uebung} \hlopt{+} \hlstd{(uebung} \hlopt{|} \hlstd{klasse),}
    \hlkwc{data} \hlstd{= beispiel_data)}
...
## -2.0982 -0.8333  0.0372  0.7253  2.8949 
## 
## Random effects:
##  Groups   Name        Variance Std.Dev. Corr
##  klasse   (Intercept)  1.1444  1.070        
##           uebung       0.0445  0.211    1.00
##  Residual             26.7805  5.175        
## Number of obs: 150, groups:  klasse, 5
## 
## Fixed effects:
##             Estimate Std. Error t value
## (Intercept)   19.993      0.962    20.8
## uebung         0.449      0.107     4.2
## 
...
Wir gelangen nun zur Analyse unseres Beispieldatensatzes. Bevor wir mit der Analyse starten können ist es wichtig, dass wir unsere Forschungsfrage klar definieren. In unserem Fall möchten wir herausfinden, wie genau sich die Prüfungsleistung von Schulkindern unter Berücksichtigung der Klassenzugehörigkeit verändert. Da wir sehr wahrscheinlich nicht nur feste Effekte sondern auch zufällige Effekte miteinander Vergleichen möchten müssen wir zudem die Modelle mit ML schätzen \citep{PEUGH201085}. Als Erstes sollte immer geprüft werden, ob eine Multilevel Analyse überhaupt nötig ist. Dies geschieht zum einen durch die Berechnung der Intraklassen Korrelation und der Überprüfung, ob überhaupt Unterschiede zwischen den Klassen besteht. Wie bereits im Abschnitt \ref{section:icc} besprochen, lässt sich die IKK aus den Varianzen des leeren Modells berechnen, das hier noch einmal kurz aufgeführt ist:
\begin{equation} \label{eq:random_intercept_slope_model}
\begin{split}	
 \text{Level 1:}  \qquad y_{ji} & = \beta_{0j} + \epsilon_{ij}\\
 \text{Level 2:} \qquad \beta_{0j} & = \gamma_{00} + U_{0j}\\
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{leeres_modell} \hlkwb{<-} \hlkwd{lmer}\hlstd{(punktzahl} \hlopt{~} \hlstd{(}\hlnum{1} \hlopt{|} \hlstd{klasse),} \hlkwc{data} \hlstd{= beispiel_data,}
    \hlkwc{REML} \hlstd{=} \hlnum{FALSE}\hlstd{)}

\hlkwd{summary}\hlstd{(leeres_modell)}
\end{alltt}
\begin{verbatim}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{lineares_modell} \hlkwb{<-} \hlkwd{lm}\hlstd{(punktzahl} \hlopt{~} \hlstd{klasse,} \hlkwc{data} \hlstd{= beispiel_data)}

\hlkwd{anova}\hlstd{(lineares_modell)}
\end{alltt}
\begin{verbatim}
...
Da nun die Frage geklärt ist, ob unser Datensatz eine Multilevel Analyse verlangt, können wir damit anfangen unser leeres Modell mit weitere Prädiktoren aufzubauen. Als erstes möchten wir die Anzahl gelöste Übungsaufgaben als festen Effekt hinzufügen. Da die Anzahl gelösten Übungsaufgaben eine Level-1 Variable ist, wird sie auch auf diesem Level der Gleichung hinzugefügt:
\begin{equation} \label{eq:random_intercept_slope_model}
\begin{split}	
...
In der oberen Gleichung kann man ebenfalls erkennen, dass es sich hier um ein einfaches \textit{Random Intercept} Modell handelt, da bei $\beta_{1}$ keine zufällige Abweichung hinzugefügt wurde. Um nun ein \textit{Random Intercept} Modell in R zu schätzen wird der Funktion die Variable wie folgt hinzugefügt:

\singlespacing
\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{modell_1} \hlkwb{<-} \hlkwd{lmer}\hlstd{(punktzahl} \hlopt{~} \hlstd{uebung} \hlopt{+} \hlstd{(}\hlnum{1} \hlopt{|} \hlstd{klasse),} \hlkwc{data} \hlstd{= beispiel_data,}
    \hlkwc{REML} \hlstd{=} \hlnum{FALSE}\hlstd{)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
Dabei ist zu beachten, dass der Term innerhalb der Klammern immer noch durch \texttt{(1 | klasse)} beschrieben wird und gemäss Tabelle \ref{tab:lmersyntax} ein \textit{Random Intercept} Modell geschätzt wird. 
...
Proportionale Reduktion der Varianz und Pseude R Squared (snijders, guide)

level-1 variablen können auch gruppen varianz erklären, wenn die komposition dieser level-1 variable relevant für gruppe ist. IQ bei vergelich von klassen mit unterschiedlichen leistungsstufen (sek und real)
