...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{beispiel_model} \hlkwb{<-} \hlkwd{lmer}\hlstd{(leistung} \hlopt{~} \hlstd{uebung} \hlopt{+} \hlstd{(uebung} \hlopt{|} \hlstd{klasse),}
                                \hlkwc{data} \hlstd{= beispiel_data)}
\hlkwd{summary}\hlstd{(beispiel_model)}
\end{alltt}
\begin{verbatim}
## Linear mixed model fit by REML ['lmerMod']
## Formula: leistung ~ uebung + (uebung | klasse)
##    Data: beispiel_data
## 
...
Dabei kann man im \texttt{summary()}-Output direkt die geschätzten Werte für die zufälligen und festen Effekte ablesen. Bei den zufälligen Effekten werden für den Achsenabschnitt als auch für die Steigung die geschätzte Varianzen, die Standardabweichungen, als auch die Korrelation dieser beiden angegeben. Der Abschnitt zu den festen Effekten ähnelt stark dem \texttt{summary()}-Output der Funktion \texttt{lm()} und lässt sich auch dementsprechend interpretieren. 

\subsubsection{Aufbau eines HLMs}

\subsubsection{Vergleich von HLMs}

\subsubsection{Auswertung von HLMs}
