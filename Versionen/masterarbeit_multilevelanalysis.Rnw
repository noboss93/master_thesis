...
\begin{equation}
\text{Relative Abweichung: } \Delta\widehat{\gamma} = \dfrac{\widebar{\widehat{\gamma}} - \gamma}{\gamma}
\end{equation}
...
Der zweite Kennwert beschreibt die Genauigkeit der Schätzung des Standardfehlers und berechnet sich aus dem Verhältnis zwischen dem mittleren Standardfehler aus einer Bedingung und der Standardabweichung der Regressionskoeffizienten über alle 1000 Replikationen dieser Bedingung \citep{hooglandboosma1998robustness, mcneish2014analyzing}. Die Formel zur Berechnung sieht wie folgt aus:
\begin{equation}
\text{SE Genauigkeit} = \dfrac{\widebar{\widehat{SE}}_{\widehat{\gamma}}}{\widehat{SD}_{\widehat{\gamma}}}
\end{equation}
Liegen Genauigkeitswerte über 1 gelten die Standardfehler als Überschätzt und liegen die Werte unter 1 werden Standardfehler unterschätzt.
Hoogland und Boomsma \citeyearpar{hooglandboosma1998robustness} bezeichnen jegliche Genauigkeitswerte, die um mehr als 0.1 von 1 abweichen als unakzeptabel. Die Genauigkeit des Standardfehlers wurde wieder in beiden Designs für jede Analysemethode und in jeder IKK Bedingung berechnet.

\subsubsection{Ergebnisse}
Der erste Kennwert der untersucht wurde, war die relative Abweichung der Regressionskoeffizienten. Die berechneten relativen Abweichungen und die Varianz dieser Abweichungen in jeder Bedingung können aus der Tabelle \ref{tab:rel_abw} entnommen werden. Sowohl bei einer Intervention auf Level-1 als auch bei einer Intervention auf Level-2 schätzten beide Modelle jegliche Regressionskoeffizienten mit einer relativen Abweichung von kleiner als $|\Delta\widehat{\gamma}| < 0.05$. Neben den Werten zur relativen Abweichung sind in Tabelle \ref{tab:rel_abw} zudem in Klammern die Varianzen der jeweiligen Regressionskoeffizienten in jeder Bedingung aufgeführt. Es zeigte sich wieder, dass bei beiden Methoden die berechneten Varianz nie grösser als $\sigma < 0.05$ (quelle) ist. 
