...
options(digits = 2)
ml_data <- readRDS(file = "dataset_theory")
model <- lmer(leistung ~ uebung + (1|klasse), data = ml_data)
...
beispiel_model <- lmer(leistung ~ uebung + (uebung | klasse), 
				data = beispiel_data)
