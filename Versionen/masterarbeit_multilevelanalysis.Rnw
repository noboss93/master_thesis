...
In den letzten Abschnitten wurde nun das Konzept und die Anwendung von hierarchsichen linearen Modellen erläutert. Im zweiten Teil dieser Arbeit wird nun eine Simulationsstudie durchgeführt, um vorhandene Forschungsergebnisse zu replizieren und aufzuzeigen, dass es Situationen gibt, bei denen es zu massiven Fehlentscheidungen kommen kann, wenn die hierarchische Struktur vernachlässigt wird. Anschliessend an diese Studie wird eine Shiny App \citep{shiny} vorgestellt, die im laufe dieser Arbeit programmiert wurde und mit der es Nutzern möglich sein wird, die Simulationsstudie aus dieser Arbeit selbst durchzuführen.
