...
Wird nun anhand dieser aggregierter Werte untersucht, wie genau die erreichte Punktzahl eines Schulkindes mit der Anzahl an gelösten Übungsaufgaben zusammenhängt, entstehen mehrere Probleme, die zu Verzerrungen und Fehlschlüssen führen können. Zum einen verändert sich die Forschungsfrage, da durch die Aggregation der Fokus von Level-1 auf Level-2 gelegt wird \citep{SnijdersTomA.B2012Ma:a, woltman2012introduction}. Mit diesen Werte untersucht man nicht mehr die erreichte Punktzahl jedes einzelnen Schülers, sondern nur noch die durchschnittlich erreichte Punktzahl pro Klasse. Das bedeutet, dass man bei den Ergebnissen nicht mehr zwischen Level-1 und Level-2 
