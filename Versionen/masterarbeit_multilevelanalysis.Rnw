...
Anhand der Abbildung \ref{fig:random_intercept_slope} lässt sich erahnen, dass das \textit{Random Intercept and Slope} Modell besser zu den Daten passt. Diese Vermutung bestätigt sich, wenn man den Residuenplot in Abbildung \ref{fig:resid_rism} betrachtet. An den beiden Endpunkten hat sich die Verteilung der Residuen stark normalisiert im Vergleich zum Residuenplot des \textit{Random Intercept} Modells aus Abbildung \ref{fig:resid_lm_rim}.  
\begin{figure}[ht!]
\centering
