...
\subsection{Studie 1: Standardfehler}
...
Um die oben genannten Aussagen zu überprüfen, wurden zwei Kennwerte berechnet. Der erste Kennwert ist die relative Abweichung der geschätzten Regressionskoeffizienten $\hat{\gamma}$ von den Populationsmittelwerten $\gamma$. Die Stärke dieser Abweichung wird in Prozent angegeben \citep{hooglandboosma1998robustness} und nach folgender Formel berechnet: 
\begin{equation}
\text{Realitve Abweichung} = \dfrac{\widebar{\widehat{\gamma}} - \gamma}{\gamma}
\end{equation}
Dabei ist $\bar{\hat{\gamma}}$ der Mittelwert aller Regressionskoeffizienten aus einer Bedingung. Diese relative Abweichung wurde in beiden Designs für jede Analysemethode in jeder IKK Bedingung für den Gesamtmittelwert $\gamma_{00}$ als auch für die Gesamtsteigung $\gamma_{10}$ resp. $\gamma_{01}$ berechnet. Gemäss Hoogland und Boomsma \citeyearpar{hooglandboosma1998robustness} gelten relative Abweichungen von kleiner als 5\% als akzeptabel.
...
\begin{equation}
\text{SE Wirksamkeit} = \dfrac{\widebar{\widehat{SE}}_{\widehat{\gamma}}}{\widehat{SD}_{\widehat{\gamma}}}
\end{equation}
...
Hoogland und Boomsma \citeyearpar{hooglandboosma1998robustness} bezeichnen jegliche Wirksamkeitswerte, die um mehr als 0.1 von 1 abweichen als unakzeptabel. Die Wirksamkeit des Standardfehlers wurde wieder in beiden Designs für jede Analysemethode in jeder IKK Bedingung berechnet.

\subsubsection{Ergebnisse}


\subsection{Studie 2: Power}
Die Satterthwaite \citeyearpar{satter1941synthesis} ist eine der diskutierten Methoden, die häufig zur Berechnung der Freiheitsgrade von hierarchischen linearen Modellen verwendet wird \citep{raudenbush2002hierarchical,SnijdersTomA.B2012Ma:a}.
