...
beispiel_model <- lmer(punktzahl ~ uebung + (uebung | klasse), data = beispiel_data)
...
\singlespacing
<<echo = TRUE, error=FALSE, warning=FALSE, message=FALSE, eval = TRUE, tidy=TRUE>>=
leeres_modell <- lmer(punktzahl ~ (1 | klasse), data = beispiel_data, REML = FALSE)

summary(leeres_modell)
@
\setstretch{1.5}
...
\singlespacing
<<echo = TRUE, error=FALSE, warning=FALSE, message=FALSE, eval = TRUE, tidy=TRUE>>=
lineares_modell <- lm(punktzahl ~ klasse, data = beispiel_data)

anova(lineares_modell)
@
\setstretch{1.5}
...
\singlespacing
<<echo = TRUE, error=FALSE, warning=FALSE, message=FALSE, eval = TRUE, tidy=TRUE>>=
modell_1 <- lmer(punktzahl ~ uebung + (1 | klasse), data = beispiel_data, REML = FALSE)
@
\setstretch{1.5}

Dabei ist zu beachten, dass der Term innerhalb der Klammern immer noch durch \texttt{(1 | klasse)} beschrieben wird und gemäss Tabelle \ref{tab:lmersyntax} ein \textit{Random Intercept} Modell geschätzt wird. Wir möchten 



Wie bei normalen linearen Modellen können diese beide Modelle nun mit dem \texttt{anova()} Befehl miteinander verglichen werden:

\singlespacing
<<echo = TRUE, error=FALSE, warning=FALSE, message=FALSE, eval = TRUE, tidy=TRUE>>=
anova(leeres_modell, modell_1)
@
\setstretch{1.5}
