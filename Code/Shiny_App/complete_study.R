...
mean_se <- function(df){
  ...
  for(i in 1:length(icc)){
  se_0_mean_lm[i] <- mean(df$SE_beta_0[test_lvl1$method == "lm" & test_lvl1$theoretical_icc == icc[i]])
  se_0_mean_mlm[i] <- mean(df$SE_beta_0[test_lvl1$method == "mlm" & test_lvl1$theoretical_icc == icc[i]])
  se_treatment_mean_lm[i] <- mean(df$SE_beta_treatment[test_lvl1$method == "lm" & test_lvl1$theoretical_icc == icc[i]])
  se_treatment_mean_mlm[i] <- mean(df$SE_beta_treatment[test_lvl1$method == "mlm" & test_lvl1$theoretical_icc == icc[i]])
  }
  ...
  colnames(mean_se_dataframe) <- c("mean_se_intercept", "mean_se_treatment", "methods", "icc")
...
sd_coefs <- function(df){
  ...
  for(i in 1:length(icc)){
    sd_0_lm[i] <- sd(df$beta_0[test_lvl1$method == "lm" & test_lvl1$theoretical_icc == icc[i]])
    sd_0_mlm[i] <- sd(df$beta_0[test_lvl1$method == "mlm" & test_lvl1$theoretical_icc == icc[i]])
    sd_treatment_lm[i] <- sd(df$beta_treatment[test_lvl1$method == "lm" & test_lvl1$theoretical_icc == icc[i]])
    sd_treatment_mlm[i] <- sd(df$beta_treatment[test_lvl1$method == "mlm" & test_lvl1$theoretical_icc == icc[i]])
  }
  ...
  colnames(mean_sd_dataframe) <- c("mean_sd_intercept", "mean_sd_treatment", "methods", "icc")
...
se_efficacy_intercept <- se_means$mean_se_intercept / coefs_sd$mean_sd_intercept
se_efficacy_treatment <- se_means$mean_se_treatment / coefs_sd$mean_sd_treatment
...
colnames(se_efficacy_df) <- c("intercept_efficacy", "treatment_efficacy", "methods", "icc")
...
ggplot(data = se_efficacy_lvl1, aes(y = intercept_efficacy, x = methods, fill = methods))+
  geom_col() +
  geom_hline(yintercept = 1, size = 1) +
  facet_grid(~ icc) + 
  labs(title = "SE Efficacy Intercept fo Treatment at Level 1")

ggplot(data = se_efficacy_lvl1, aes(y = treatment_efficacy, x = methods, fill = methods))+
  geom_col() +
  geom_hline(yintercept = 1, size = 1) +
  facet_grid(~ icc) +
  labs(title = "SE Efficacy Treatment fo Treatment at Level 1")

ggplot(data = se_efficacy_lvl2, aes(y = intercept_efficacy, x = methods, fill = methods))+
  geom_col() +
  geom_hline(yintercept = 1, size = 1) +
  facet_grid(~ icc) +
  labs(title = "SE Efficacy Intercept fo Treatment at Level 2")

ggplot(data = se_efficacy_lvl2, aes(y = treatment_efficacy, x = methods, fill = methods))+
  geom_col() +
  geom_hline(yintercept = 1, size = 1) +
  facet_grid(~ icc) +
  labs(title = "SE Efficacy Treatment fo Treatment at Level 2")
